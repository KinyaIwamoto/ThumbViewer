...
//class CDirTreeCtrl : public CMFCShellTreeCtrl
{
// Construction
public:
	CDirTreeCtrl();
...
//	CString GetFullPath (HTREEITEM item);
	...
	//void RefreshSubItems(HTREEITEM hParent, NM_TREEVIEW* pNMTreeView);
...
private:
	...
	void ExpandItem(HTREEITEM hItem, NM_TREEVIEW* pNMTreeView = NULL);		// after expanded

	BOOL NeedsChildren(HTREEITEM hParent);	// true if no child items
	void DeleteChildren(HTREEITEM hParent);

	void PopulateRoot();
	//void PopulateRoot(NM_TREEVIEW* pNMTreeView);
	BOOL WantsRefresh(HTREEITEM hItem);

	//BOOL PopulateItem(HTREEITEM hParent);
	BOOL PopulateItem(HTREEITEM hParent, NM_TREEVIEW* pNMTreeView);
