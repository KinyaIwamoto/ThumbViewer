...
UINT CThumbViewerView::LoadThumbNail(LPVOID lpParam)
{
	...
	vector<CString> vFileName = pDoc->m_vFileName;	//20200512 add m_vFileNameを直接スレッド内で操作するとメモリリークでm_vFileNameを破壊する
...
void CThumbViewerView::LoadThumbImages()
{
	...
	m_hShoriFlag = ::CreateFileMapping( INVALID_HANDLE_VALUE,
										...
										PAGE_READWRITE,
										0,
										sizeof(BOOL),		// 共有メモリのサイズ
										_T("SHORIFLAG") );	// 共有メモリ名
	if( m_hShoriFlag == NULL ) {
		AfxMessageBox(_T("CreateFileMappingエラー"));
		return;
	}
	...
	if( m_lpShoriFlag == NULL){
		AfxMessageBox(_T("MapViewOfFileエラー"));
		return;
	}
	...
	*m_lpShoriFlag = TRUE;
	...
	// m_hEventStop = ::CreateEvent(NULL, TRUE, FALSE, NULL);
		m_pRecvThread = NULL; // スレッドオブジェクトのポインタクリア
	...
	m_pRecvThread->m_bAutoDelete = FALSE;	// 自動破棄フラグクリア
	m_pRecvThread->ResumeThread();			// サスペンド解除
	//20200512 add ここまで
...
BOOL CThumbViewerView::TerminateThread()
{
	if ( !m_bRunning )
	{
		...
		// 共有メモリの破棄
		if(m_lpShoriFlag){
			::UnmapViewOfFile(m_lpShoriFlag);		// 共有メモリのclose
			::CloseHandle( m_hShoriFlag);			 // 共有メモリの削除
			m_lpShoriFlag = NULL;
			m_hShoriFlag = NULL;
		}
