...
protected:
	...
	CWinThread* m_pRecvThread;	// 20200512 add スレッドのオブジェクト
	HANDLE m_hShoriFlag;		// 20200512 add 処理フラグ用共有メモリへのハンドル
	BOOL* m_lpShoriFlag;		// 20200512 add 処理フラグ用共有メモリ
	static UINT LoadThumbNail(LPVOID lpParam);	//スレット処理本体
	//static unsigned __stdcall	LoadThumbNail(LPVOID lpParam);	//スレット処理本体
